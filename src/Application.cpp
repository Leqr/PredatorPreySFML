...
Vec2d getWindowSize()
{
    auto width = getAppConfig().window_simulation_width + getAppConfig().window_info_width;
...
Vec2d getInfoSize() {
    auto width = getAppConfig().window_info_width;
    auto height = getAppConfig().window_simulation_height;
    return { width, height };
}

Vec2d getInfoPosition(){
    return {getSimulationSize().x,0};
}
...
void Application::run()
{
    ...
    // Create the Stats background
    auto statsBackground = sf::RectangleShape();
    statsBackground.setSize(getStatsSize());
    statsBackground.setFillColor(sf::Color(985, 985, 985));

    //Create the Info Background
    auto infoBackground = sf::RectangleShape();
    infoBackground.setSize(getInfoSize());
    infoBackground.setFillColor(sf::Color(985, 985, 985));
    ...
    while (mRenderWindow.isOpen()) {
       ...
       //getStats().setActive(activeIndex);
        ...
        // Render everything
        render(mSimulationBackground, statsBackground, infoBackground);
...
void Application::drawOnInfoPanel(sf::RenderTarget& target) {
    const double gapX = getAppConfig().window_info_width/12;                       //set the gap with the border of the info frame
    const double gapY = getAppConfig().window_simulation_height/60;
    const int FONT_SIZE = getAppConfig().window_simulation_height/60;
    const int SMALL_FONT_SIZE = getAppConfig().window_simulation_height/100;

    //KEYS
    target.draw(buildText("KEYS : ", {gapX, 10}, getAppFont(), FONT_SIZE + 2, sf::Color::Red));
    target.draw(buildText("1 or 2 : New Sheep", {gapX, 3*gapY}, getAppFont(), FONT_SIZE, sf::Color::Black));
    target.draw(buildText("W     :  New Wolf", {gapX, 6*gapY}, getAppFont(), FONT_SIZE, sf::Color::Black));
    target.draw(buildText("P     :  New Random Plant", {gapX, 9*gapY}, getAppFont(), FONT_SIZE, sf::Color::Black));
    target.draw(buildText("N     :  New Plant", {gapX, 12*gapY}, getAppFont(), FONT_SIZE, sf::Color::Black));
    target.draw(buildText("T     :  New Toxic Plant", {gapX, 15*gapY}, getAppFont(), FONT_SIZE, sf::Color::Black));
    target.draw(buildText("V     :  New Virtuous Plant", {gapX, 18*gapY}, getAppFont(), FONT_SIZE, sf::Color::Black));
    target.draw(buildText("C     :  New Cacti", {gapX, 21*gapY}, getAppFont(), FONT_SIZE, sf::Color::Black));
    target.draw(buildText("I     :  Infect An Animal", {gapX, 24*gapY}, getAppFont(), FONT_SIZE, sf::Color::Black));
    target.draw(buildText("F     :  Individual Stats", {gapX, 27*gapY}, getAppFont(), FONT_SIZE, sf::Color::Black));
    target.draw(buildText("Z     :  General Stats", {gapX, 30*gapY}, getAppFont(), FONT_SIZE, sf::Color::Black));
    target.draw(buildText("R     :  Reset", {gapX, 33*gapY}, getAppFont(), FONT_SIZE, sf::Color::Black));
    target.draw(buildText("D     :  Debug Mode", {gapX, 36*gapY}, getAppFont(), FONT_SIZE, sf::Color::Black));
    target.draw(buildText("S      :  Zoom In/Out Stat", {gapX, 39*gapY}, getAppFont(), FONT_SIZE, sf::Color::Black));
    target.draw(buildText("SPACE :  Pause", {gapX, 42*gapY}, getAppFont(), FONT_SIZE, sf::Color::Black));
    target.draw(buildText("Scroll :  Zoom In/Out Sim", {gapX, 45*gapY}, getAppFont(), FONT_SIZE, sf::Color::Black));
    target.draw(buildText("Arrows :  Move the View", {gapX, 48*gapY}, getAppFont(), FONT_SIZE, sf::Color::Black));

    //name
    target.draw(buildText("Camille Ohlmann & Leonard Equer, 2017, EPFL", {gapX, getAppConfig().window_simulation_height - 2*gapY},
    getAppFont(), SMALL_FONT_SIZE, sf::Color::Black));
}

sf::Text Application::buildText(std::string const& msg, Vec2d const& position, sf::Font const& font, unsigned int size, sf::Color color){
...
void Application::drawBorder(sf::RenderTarget& target){
    float statvsinfoHeight = getAppConfig().window_simulation_height/300;;
    float statvsinfoWidth = getAppConfig().window_simulation_width;
    auto statvsinfo = sf::RectangleShape({statvsinfoWidth, statvsinfoHeight});
    float borderYPos = getAppConfig().window_simulation_height - statvsinfoHeight;
    statvsinfo.setPosition({0,borderYPos});
    statvsinfo.setFillColor(sf::Color(600,600,600));
    target.draw(statvsinfo);
}
...
void Application::createViews()
{
    ...
    mStatsView = setupView(getStatsSize(),
                           ...
                           mRenderWindow.getSize());
    mInfoView = setupView(getInfoSize(), getInfoPosition(),
    getInfoSize(), mRenderWindow.getSize());
}
...
void Application::render(sf::Drawable const& simulationBackground, sf::Drawable const& statsBackground, sf::Drawable const& infoBackground)
{
    mRenderWindow.clear();
    ...
    mRenderWindow.setView(mInfoView);
    mRenderWindow.draw(infoBackground);
    drawOnInfoPanel(mRenderWindow);
    drawBorder(mRenderWindow);
