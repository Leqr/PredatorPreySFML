...
public:
     ...
     */
    Graph(std::vector<std::string> const& titles, Vec2d const& size, double min, double max,ZOOM zoom = MAX);
    ...
    void changeGraphScale();                //Changes the graph's zoom and reset

    double getScalingRatio();
...
private:
    ...
    ZOOM zoom;                            ///Allow two types of scaling.
};
