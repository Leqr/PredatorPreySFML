...
std::vector<sf::Color> const& COLORS = { sf::Color(153,0,0), sf::Color::Black, sf::Color(0,153,0), sf::Color(204,0,102),
                                         sf::Color(0,204,204), sf::Color::Magenta
                                       };

} // anonymous

Graph::Graph(std::vector<std::string> const& titles, Vec2d const& size, double min, double max, ZOOM zoom)
    : mSize(size)
...
void Graph::updateData(sf::Time deltaEpoch, std::unordered_map<std::string, double> const& newData) {
    ...
    auto const Y_SCALE = 1.0 * getScalingRatio() / (mYMax - mYMin) * mSize.y;
...
void Graph::drawOn(sf::RenderTarget& target) const {
    ...
    auto const FONT_SIZE = getAppConfig().window_simulation_height/60;
    ...
    for (auto const& serie : mSeries) {
        ...
        if((serie.title == s::WOLVES)or(serie.title == s::SHEEP)or(serie.title == s::PLANT)or(serie.title == s::INFECTED)){
            tmpStream << std::fixed << std::setprecision(0) << serie.lastValue;
        } else {
...
double Graph::getScalingRatio(){
    if(zoom == MAX){
        return 1;
    } else if (zoom == MIN){
        return getAppConfig().stats_upscaling_ratio;
    }
}
